...
export default function AdminForget() {
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest('/admin/forget', {
        method: 'POST',
        body: data,
      });
      ...
      if (response && response.message) {
        ...
        if (response.message.includes('success-box') || response.message.includes('success')) {
          addAlert(text || 'OTP sent successfully!', 'success');
          setStep(2);
        } else {
          addAlert(text || 'Verification failed.', 'danger');
        }
      }
    } catch (err) {
      console.error(err);
      addAlert('Verification failed. Email may not exist.', 'danger');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (response && response.message) {
        ...
        if (response.message.includes('success-box') || response.message.includes('success')) {
          ...
          addAlert(text || 'Invalid OTP.', 'danger');
        }
      }
    } catch (err) {
      console.error(err);
      addAlert('Verification failed. Invalid OTP.', 'danger');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdatePassword = async (e) => {
    ...
    try {
      ...
      if (response && response.status === 'redirect') {
        ...
        const text = parseDjangoMessage(response.message);
        if (response.message.includes('success-box') || response.message.includes('success')) {
          addAlert(text || 'Password updated successfully.', 'success');
      ...
      console.error(err);
      addAlert('Failed to reset password.', 'danger');
    } finally {
      setLoading(false);
    }
  };
