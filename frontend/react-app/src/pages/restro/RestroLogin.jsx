...
export default function RestroLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiRequest('/restro/login', {
        method: 'POST',
        body: data,
      });

      if (response && response.status === 'redirect') {
        addAlert('Restaurant logged in successfully!', 'success');
        // Fetch session data to update frontend context user state
        const sessData = await apiRequest('/menu/');
        if (sessData && sessData.user) {
          setUser({
            username: sessData.user,
            name: sessData.name || sessData.user,
            role: sessData.role,
          });
        }
        ...
        window.location.href = '/restro/';
      }
    } catch (err) {
      console.error(err);
      addAlert('Invalid restaurant username or password.', 'danger');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="auth-shell">
        ...
        <h1 className="auth-title">Restaurant Panel</h1>
        <p className="auth-subtitle">Login to manage your restaurant menu, orders, and sales</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="auth-helpers">
            <div></div>
            <Link className="link-inline" to="/restro/forget">Forgot Password?</Link>
          </div>
        ...
        <div className="auth-footer">
          Partner with us? <Link className="link-inline" to="/restro/signup">Register Restaurant</Link>
        </div>
      </div>
    </>
  );
